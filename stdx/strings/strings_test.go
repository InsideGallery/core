...
func TestSimHash(t *testing.T) {
	testcases := []struct {
		...
		{
			input:   []byte("test strings"),
			output:  "dcddff7fbffebf79",
			input2:  []byte("another strings"),
			compare: 16,
		},
		{
			input:   []byte("sim strings"),
			output:  "dcdffb5b6f7fef3b",
			input2:  []byte("sime strings"),
			compare: 13,
		},
		{
			input:   []byte("different strings"),
			output:  "fcdfdffb677e2fb9",
...
func TestMaskField(t *testing.T) {
	testcases := []struct {
		...
		{
			name:              "mask long strings",
			str:               "secret-strings-here",
			keepUnmaskedFront: 2,
			keepUnmaskedEnd:   3,
			expected:          "se**************ere",
		},
		{
			name:              "mask short strings",
			str:               "sec",
			keepUnmaskedFront: 2,
			keepUnmaskedEnd:   3,
			expected:          "***",
		...
		{
			name:              "mask minimum to have show strings",
			str:               "secret12345",
			keepUnmaskedFront: 2,
			keepUnmaskedEnd:   3,
			expected:          "se******345",
