...
func TestOutOfBoundsLong64(_ *testing.T) {
	v := New64(64)
	v.Set(1000)
}

func TestOutOfBoundsClose64(_ *testing.T) {
	v := New64(65)
	v.Set(66)
}
...
func TestDump64(t *testing.T) {
	a := New64(100)
	for i := uint64(1); i < 100; i += 2 {
		a.Set(i)
	}

	testutils.Equal(t, a.Count(), uint64(50))

	raw, err := a.ToBytes()
	testutils.Equal(t, err, nil)

	b, err := NewFromBytes64(raw)
	testutils.Equal(t, err, nil)

	testutils.Equal(t, b.Count(), uint64(50))
	testutils.Equal(t, a.Difference(b).Count(), uint64(0))
}
